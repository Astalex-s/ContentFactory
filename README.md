...
**ContentFactory** — веб-приложение для автоматической генерации маркетингового контента для товаров маркетплейса. Система загружает товары, генерирует текстовые описания, изображения и видео с помощью AI-моделей, а затем публикует контент в социальные сети (YouTube, VK).
...
2. **Генерация текстового контента** — посты, видеоописания, CTA для YouTube и VK с учётом тона и платформы
...
5. **Подключение социальных сетей** — OAuth-авторизация для YouTube и VK с поддержкой нескольких каналов/аккаунтов
...
> **Важно:** OAuth-приложения (client_id, client_secret для YouTube/VK) **не хранятся** в `.env`.
...
### Social (`/social`)
- `GET /social/connect/{platform}` — URL для OAuth (youtube, vk)
...
- **Текст:** посты, видеоописания, CTA — с выбором платформы (YouTube/VK) и тона (профессиональный/дружелюбный/продающий)
...
| [docs/SOCIAL_PLATFORMS.md](docs/SOCIAL_PLATFORMS.md) | Настройка OAuth для YouTube и VK (пошаговая инструкция) |
