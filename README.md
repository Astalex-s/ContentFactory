# ContentFactory
Ссылка на канал Телеграм: https://t.me/portfolio_Alexey_Astafyev/30

**ContentFactory** — веб-приложение для автоматической генерации маркетингового контента для товаров маркетплейса. Система загружает товары, генерирует текстовые описания, изображения и видео с помощью AI-моделей, а затем публикует контент на YouTube.
...
2. **Генерация текстового контента** — видеоописания, CTA для YouTube с учётом тона
...
5. **Подключение YouTube** — OAuth-авторизация с поддержкой нескольких каналов/аккаунтов
...
| `REPLICATE_VIDEO_SEGMENTS` | `1` | Количество сегментов (5 = длинное видео из 5 частей) |
| `REPLICATE_VIDEO_SEGMENT_DURATION` | `5` | Длительность одного сегмента (сек) |
| `TTS_PROVIDER` | — | Озвучка: `openai` или `edge` (пусто = без озвучки) |
...
`REPLICATE_VIDEO_SEGMENTS`, `REPLICATE_VIDEO_SEGMENT_DURATION`, `TTS_PROVIDER` подтягиваются только из `.env` через `env_file` в docker-compose (не дублируются в блоке `environment`).

> **Важно:** OAuth-приложения (client_id, client_secret для YouTube) **не хранятся** в `.env`.
...
### Social (`/social`)
- `GET /social/connect/{platform}` — URL для OAuth (youtube)
...
- `POST /analytics/refresh-stats` — массовое обновление статистики из YouTube (для cron)
...
- **Текст:** видеоописания, CTA — с выбором тона (профессиональный/дружелюбный/продающий)
...
# Установить hooks: pre-commit (black форматирует код) + pre-push (проверки)
./scripts/install-hooks.sh
```

Скрипт `scripts/refresh_yt_stats.sh` — обновление статистики YouTube (POST /analytics/refresh-stats). Для cron на сервере: `*/15 * * * * CONTENTFACTORY_URL=http://127.0.0.1:8000 ./scripts/refresh_yt_stats.sh`
...
| [docs/SOCIAL_PLATFORMS.md](docs/SOCIAL_PLATFORMS.md) | Настройка OAuth для YouTube (пошаговая инструкция) |
