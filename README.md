...
1. Пользователь выбирает видео и нажимает "Опубликовать"
2. Заполняет форму:
   - Выбор платформы и аккаунта
   - Заголовок (можно сгенерировать через AI или использовать название товара)
   - Описание (можно выбрать из сгенерированных текстов)
   - Время публикации (опционально)
3. `POST /publish/{content_id}` создаёт запись в очереди
4. Фоновая задача обрабатывает публикацию:
   - Проверка существования файла
   - Обновление токена при необходимости (YouTube токены живут ~1 час)
   - Для длинных видео (≥60 сек): добавление ссылки на товар в описание
   - Для всех видео: добавление QR-кода с ссылкой на товар в конец видео (2.5 сек)
   - Загрузка через провайдер платформы
   - Обновление статуса в БД
5. `GET /publish/status/{id}` — проверка статуса публикации
