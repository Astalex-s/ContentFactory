...
## ЭТАП 8 — Подключение аккаунтов: настройки и мульти-OAuth

### Концепция

Переработка функционала подключения аккаунтов YouTube, VK, TikTok: поддержка нескольких OAuth-приложений (клиентских учётных данных) на платформу, единая страница настроек для ввода данных подключений, страница выбора и подключения аккаунтов с переходом в настройки для добавления нового приложения. Все учётные данные OAuth-приложений (client_id, client_secret и при необходимости дополнительные поля) хранятся только в БД в зашифрованном виде. В .env — только данные, которые не меняются при подключении дополнительных аккаунтов (ключ шифрования и т.п.); дефолтных подключений из .env нет — их можно отключить на данном этапе и позже подключить в общем формате (та же таблица, тот же API). Соблюдать PROJECT_RULES.

---

### 8.1 Переменные окружения и политика хранения

- **В .env только то, что не меняется при подключении дополнительных аккаунтов:** ключ шифрования (OAUTH_SECRET_KEY или OAUTH_ENCRYPTION_SALT), при необходимости базовый URL приложения для redirect_uri. Никаких client_id, client_secret для YouTube/VK/TikTok в .env не хранить.
- **Дефолтные подключения:** не использовать. На данном этапе все OAuth-приложения задаются через настройки и хранятся в БД. Позже «дефолтное» или глобальное подключение можно ввести в том же формате (та же таблица, тот же CRUD, опционально user_id=null или флаг is_default).
- **Шифрование данных в БД:** использовать существующий механизм (Fernet, ключ из .env). Все учётные данные OAuth-приложений — только в БД в зашифрованном виде.

---

### 8.2 Модель и хранение кастомных OAuth-приложений (backend)

Создать модель для хранения учётных данных дополнительных OAuth-приложений. Таблица (например `oauth_app_credentials` или `social_oauth_apps`): id UUID primary key, user_id FK (владелец или null для глобальных), platform enum (youtube, vk, tiktok), name string (название приложения для отображения), client_id string, client_secret encrypted (шифровать при записи, расшифровывать только при использовании в OAuth flow), optional: redirect_uri, created_at, updated_at. Индексы по platform, user_id. Миграция Alembic. Репозиторий: create, get_by_id, list_by_platform, list_all, update, delete. При чтении client_secret отдавать в сервис уже расшифрованным только для нужд OAuth; в API и логах никогда не возвращать и не логировать.

---

### 8.3 Сервис и шифрование (backend)

Реализовать OAuthAppCredentialsService (или расширить OAuthService). Методы: create_credentials(platform, name, client_id, client_secret, ...), update_credentials(id, ...), list_credentials(platform?), delete_credentials(id). Шифрование client_secret при сохранении через существующий encrypt_token/decrypt_token (или отдельные методы для app credentials). Проверка прав: только владелец или админ может изменять/удалять. Валидация: platform из enum, name не пустой, client_id/client_secret не пустые. Логика только в Service; слой Repository для БД. Не логировать и не возвращать в ответах client_secret в открытом виде.

---

### 8.4 API для кастомных OAuth-приложений (backend)

Эндпоинты: GET /social/oauth-apps — список кастомных приложений (query: platform?); POST /social/oauth-apps — создание (body: platform, name, client_id, client_secret, redirect_uri?); PATCH /social/oauth-apps/{id} — обновление (частичное); DELETE /social/oauth-apps/{id} — удаление. В ответах списка и деталей не возвращать client_secret (или возвращать маскированное, например "••••••••"). Request/Response — Pydantic-схемы (Create, Update, Read). Проверка авторизации; при необходимости проверка роли (только админ или владелец). Единый формат ошибок (code/message/details). Без бизнес-логики в router.

---

### 8.5 Выбор OAuth-приложения при подключении аккаунта (backend)

При инициации OAuth (например GET /social/connect/{platform} или аналог): обязательный query-параметр oauth_app_id. Учётные данные брать только из таблицы OAuth-приложений в БД (client_id, расшифрованный client_secret, при необходимости redirect_uri). Fallback на .env не использовать — дефолтных подключений нет. Сохранять в social_accounts связь с oauth_app_credentials_id (FK), чтобы знать, через какое приложение подключён аккаунт. Обработка ошибок: 404 если oauth_app_id не найден или не принадлежит пользователю; 400 если платформа не совпадает или oauth_app_id не передан.

---

### 8.6 Страница настроек (frontend)

Переработать страницу Settings (или раздел «Подключения» / «Соцсети»): блок «Дополнительные OAuth-приложения». Форма добавления: выбор платформы (YouTube / VK / TikTok), название приложения, Client ID, Client Secret, при необходимости Redirect URI. Кнопки: «Сохранить», «Отмена». Список уже добавленных приложений: платформа, название, маскированный client_id (например последние 4 символа), действия «Изменить» / «Удалить» (с подтверждением). Валидация полей на фронте; ошибки API отображать в едином формате. Данные для подключения не хранить в localStorage в открытом виде; после успешной отправки на backend форма очищается. Использовать UI-Kit (PROMPTS/UI_KIT.md), feature-based структура (например features/settings/ или features/social/).

---

### 8.7 Страница подключения аккаунтов (frontend)

На странице подключения аккаунтов (Creators или отдельная «Аккаунты»): кнопка «Подключить аккаунт» — выбор платформы и выбор OAuth-приложения из списка сохранённых в настройках (все данные из БД; дефолтного из .env нет). После выбора — переход в OAuth flow (как в Этапе 4). Если приложений для платформы нет — показывать подсказку и кнопку «Добавить OAuth-приложение» с переходом в настройки. Кнопка «Добавить новое OAuth-приложение» / «Настроить приложение» — переход на страницу настроек (блок с формой добавления OAuth-приложений). Список уже подключённых аккаунтов: платформа, название/имя аккаунта, через какое приложение подключён, действия «Отключить». Состояния загрузки и ошибок; единый формат ошибок с бэкенда.

---

### 8.8 Связь настроек и списка аккаунтов

После добавления или редактирования OAuth-приложения в настройках пользователь может вернуться на страницу подключения аккаунтов и выбрать это приложение при нажатии «Подключить аккаунт». Не дублировать ввод client_id/client_secret на странице аккаунтов — только выбор из списка сохранённых приложений и переход в настройки по кнопке «Добавить новое приложение».

---

### 8.9 Переменные окружения (Этап 8)

В .env только данные, не зависящие от конкретного подключения: OAUTH_SECRET_KEY или OAUTH_ENCRYPTION_SALT (ключ шифрования для токенов и учётных данных в БД), при необходимости BASE_URL или FRONTEND_URL для redirect_uri. Переменные YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, VK_*, TIKTOK_* для учётных данных не использовать — все OAuth-приложения настраиваются через UI и хранятся в БД. В .env.example и README перечислить только эти переменные; дефолтные подключения не документировать (отключены, позже — в общем формате через тот же API).

---

### 8.10 Аудит этапа 8

Проверить: в .env только неизменяемые при подключении данные (ключ шифрования и т.д.), без client_id/client_secret платформ; все учётные данные OAuth-приложений только в БД, в зашифрованном виде; в API и логах client_secret не возвращается и не логируется; Router без бизнес-логики; Service/Repository разделены; при подключении аккаунта обязателен oauth_app_id, fallback на .env отсутствует; фронт: настройки — ввод и список OAuth-приложений, страница аккаунтов — подключение с выбором приложения из списка, кнопка перехода в настройки; единый формат ошибок; соответствие PROJECT_RULES и UI-Kit. Обновить docs (например docs/SOCIAL_PLATFORMS.md): хранение OAuth-приложений в БД, поток «подключить аккаунт» с выбором приложения, отсутствие дефолтных подключений из .env.
